...
export class AuthService {
  ...
  async telegramLogin(data: string): Promise<string> {
    ...
    /**
     * Store all needed data in the database to show organizer name and photo in the app.
     */
    ...
    const tokenPayload = JSON.stringify({
      sub: user.id,
      // queryId is used to send a message to the chat when attaching event
      queryId: parsed.query_id,
      // language code is used to format dates when sending a message with an event to the chat
